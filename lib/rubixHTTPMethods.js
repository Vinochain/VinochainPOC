...
/*            
	...
	Meteor.methods({
		...
		'createWallet': function(pass){
			...
			var response = lightwallet.keystore.deriveKeyFromPassword(pass, function(error, pwDerivedKey){
				if(error){
					...
					global_keystore.generateNewAddress(pwDerivedKey, numAddr, undefined, function(err, res){
				if(err){
					throw err;
				} else if(res){
					return res;
		...
		'getNonce': function(address){
				...
				res = web3.eth.getTransactionCount(address, function(error,result){
					if(error){
						throw error;
					} else if(result){
						return result;
		...
		sendValue: function(keystore, pass, txOptions){
			...
			var txh = Web3.eth.sendRawTransaction(fut.wait());
			var txh = JSON.parse(res.content);
			var hashAttributes = {
				txHash: txh,
				from: signingAddress
		...
		sendFunctionWithOneArg: function(keystore, pass, contractAddress, nonce, functionName, arg){
			...
			signingAddress = keystore["m/0'/0'/0'"].addresses[0];
			var deriv = lightwallet.keystore.deriveKeyFromPassword(pass);
			var ret = new Uint8Array(32);
			for (var i = 0; i < 32; i++) {
				ret[i] = deriv[i];
...
//      var stateGetter = lightwallet.signing.signTx(global_keystore, ret, stateTx, signingAddress, undefined);
			var res = Web3.eth.sendRawTransaction("0x" + fut.wait());
			var txh = JSON.parse(res.content);
			var hashAttributes = {
				txHash: txh,
//        nxtTx: stateGetter,
...
//        nxtTx: stateGetter,
		...
		createBottle: function(keystore, pass, nonceOfTx){
			...
			// var keyst = lightwallet.upgrade.upgradeOldSerialized(keystore, "password");
			var global_keystore = lightwallet.keystore.deserialize(keystore);
			var signingAddress = keystore["m/0'/0'/0'"].addresses[0];
			var deriv = lightwallet.keystore.deriveKeyFromPassword(pass);
			var ret = new Uint8Array(32);
			for (var i = 0; i < 32; i++) {
				ret[i] = deriv[i];
			...
			var res = HTTP.post('http://192.168.87.202:3000/web3/sendRawTransaction', {data:{"txRaw": fut.wait()}});
			var txh = JSON.parse(res.content);
			var hashAttributes = {
				txHash: txh,
				from: signingAddress
